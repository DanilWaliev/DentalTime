...
import { CancelAppointment, GetAppointmentByNumber, GetAppointments, RescheduleAppointment } from './../data/data.js';
...
function init() {
    ...
    bindAction(statusCard, 'cancel', handleCancel);
    bindAction(statusCard, 'reschedule', handleReschedule);
}
...
function handleReschedule(appointmentId) {
    ...
    showRescheduleDialog(appointment, (newDate) => {
        const updatedAppointment = RescheduleAppointment(appointmentId, newDate);
        renderCurrentAppointment(updatedAppointment);
